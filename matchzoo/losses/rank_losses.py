...
mz_specialized_losses = {'rank_hinge_loss', 'rank_crossentropy_loss', 'ranknet'}
...
def ranknet(kwargs=None):
    def _margin_loss(y_true, y_pred):
        return K.mean(K.log(1. + K.exp(-(y_true * y_pred[:,1] - (1-y_true) * y_pred[:,1]))), axis=-1)
    return _margin_loss
