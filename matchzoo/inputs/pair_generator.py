...
class PairGenerator(PairBasicGenerator):
    ...
    def get_batch_static(self):
        ...
        for i in range(self.batch_size):
            ...
            ID_triplets.append((d1, d2p, d2n))
        return X1, X1_len, X2, X2_len, Y, ID_triplets
    ...
    def get_batch_iter(self):
        while True:
            ...
            for _ in range(self.config['batch_per_iter']):
                ...
                yield X1, X1_len, X2, X2_len, Y, ID_triplets

    def get_batch_generator(self):
        while True:
            X1, X1_len, X2, X2_len, Y, ID_triplets = self.get_batch()
            if self.config['use_dpool']:
                ...
                yield ({'query': X1, 'query_len': X1_len, 'doc': X2, 'doc_len': X2_len, 'ID': ID_triplets}, Y)
