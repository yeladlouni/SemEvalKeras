...
class Preparation(object):
    ...
    * Relation File: this file records the relationship between two texts, each line containing the label and a pair of ids.
    ...
    def parse_line(self, line, delimiter='\t'):
        ...
        # print('subs: ', len(subs))
        if len(subs) not in (3, 5):
            raise ValueError('format of data file wrong, should be \'label,text1,text2\' or \'label,text1,text2,qid,qaid\'.')
        elif 5 == len(subs):
    ...
    def run_with_one_corpus(self, file_path):
        ...
        for line in f:
            ...
            if len(self.parse_line(line)) == 5:
                    label, t1, t2, qid, qaid = self.parse_line(line)
                    id1 = qid
                    id2 = qaid
            else:
                label, t1, t2 = self.parse_line(line)
                id1 = self.get_text_id(hashid_q, t1, 'T')
                id2 = self.get_text_id(hashid_d, t2, 'T')
            corpus[id1] = t1
            corpus[id2] = t2
