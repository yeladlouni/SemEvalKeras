...
def train(config):
    ...
    with open('./examples/semeval/logs/' + config['net_name'] + '.log', 'w') as f:
        for i_e in range(num_iters):
            ...
            for tag, generator in eval_gen.items():
                ...
                f.write('Iter:%s\t%d\t%s\n' % (tag, i_e, '\t'.join(['%s=%f'%(k,v/num_valid) for k, v in res.items()])))
                sys.stdout.flush()
...
def predict(config):
    ...
    for tag, generator in predict_gen.items():
        ...
        if tag in output_conf:
            file_name = config['net_name'] + '_' + time.strftime('%Y%d%m%H%M%S', time.localtime(time.time()))
            if output_conf[tag]['save_format'] == 'TREC':
                save_path = os.path.join(output_conf[tag]['save_dir'], file_name + '.TREC')
                ...
                save_path = os.path.join(output_conf[tag]['save_dir'], file_name + '.SEMEVAL')
                with open(save_path, 'w') as f:
                    temp = []
                    ...
                    for qid, dinfo in res_scores.items():
                            ...
                            for rank,(qaid, (score, rel)) in enumerate(dinfo):
                                temp.append([qid[1:], qaid[2:], rank, score, 'true' if rank < 3 else 'false'])
                ...
                save_path = os.path.join(output_conf[tag]['save_dir'], file_name + '.TEXTNET')
                with open(save_path, 'w') as f:
                    for qid, dinfo in res_scores.items():
                        dinfo = sorted(dinfo.items(), key=lambda d:d[1][0], reverse=True)
                        for inum,(did, (score, gt)) in enumerate(dinfo):
